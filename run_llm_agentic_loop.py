...
from agent.llm_planner import propose_next_config
...
def save_history(history):
    ...
    with open("results/llm_agentic_history.json", "w") as f:
...
def main():
    ...
    for round_id in range(1, MAX_ROUNDS + 1):
        print("=" * 80)
        print(f"LLM Agentic AutoRL Round {round_id}")

        config = propose_next_config(history)

        print("LLM proposed config:")
        ...
        if result["mean_reward"] >= TARGET_REWARD and round_id >= MIN_ROUNDS:
            print("=" * 80)
            print(f"Target reached after minimum rounds: mean_reward = {result['mean_reward']}")
            break

    print("=" * 80)
    print("Final LLM experiment history:")
